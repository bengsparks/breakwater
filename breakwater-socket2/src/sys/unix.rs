...
/// ```console
/// readelf -Ws --dyn-syms $(dirname $LIB)/vcl_ldpreload.so | grep $SYMBOL
/// ```
mod vpp {
    use libc::{
        c_int, c_void, msghdr, nfds_t, off_t, pollfd, size_t, sockaddr, socklen_t, ssize_t,
    };

    #[link(name = "vcl_ldpreload")]
    unsafe extern "C" {
